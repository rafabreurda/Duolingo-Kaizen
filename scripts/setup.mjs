// setup.mjs — executa todo o setup do Kaizen Dojo no Supabase
import { createClient } from '@supabase/supabase-js'

const URL  = 'https://vptxklxiscmxrcmkckk.supabase.co'
const ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZwdHhrbHhpc2NteHJjbWtja2trIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM4OTUzOTUsImV4cCI6MjA4OTQ3MTM5NX0.LPsWfscfsf-2YoinWFwgwTxLSlHc8-wO0Qmm8TkUzwQ'
...
const db = createClient(URL, SRVC, {
  auth: { autoRefreshToken: false, persistSession: false }
})

// ── executa SQL via Management API ────────────────────────────────────────────
async function sql(query) {
  const res = await fetch(`https://api.supabase.com/v1/projects/vptxklxiscmxrcmkckk/database/query`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${SRVC}`,
    },
    body: JSON.stringify({ query }),
  })
  const json = await res.json()
  if (!res.ok) {
    // Tenta via endpoint alternativo
    ...
    if (!res2.ok) throw new Error(json.message || JSON.stringify(json))
  }
  return json
}
...
async function criarAdmin(nome, username, senha, role) {
  ...
  if (error) {
    if (error.message.includes('already been registered') || error.message.includes('already exists')) {
      console.log(`  ⚠  ${nome} já existe — atualizando role...`)
      // Tenta atualizar o role
      const { data: existing } = await db
        .from('username_lookup').select('user_id').eq('username', username).single()
      if (existing) {
        await db.from('perfis').update({ role, nome, username }).eq('id', existing.user_id)
        console.log(`  ✓  ${nome} atualizado`)
      }
      return
    }
    throw new Error(`Erro ao criar ${nome}: ${error.message}`)
  }
  const uid = data.user.id
  await db.from('perfis').update({ role, nome, username }).eq('id', uid)
  await db.from('username_lookup').upsert({ username, user_id: uid })
  console.log(`  ✓  ${nome} criado  (login: ${username}  |  email interno: ${email(username)})`)
}
...
async function main() {
  ...
  // 1. Tenta criar o schema via Management API
  console.log('1. Criando schema no Supabase...')
  try {
    const res = await fetch('https://api.supabase.com/v1/projects/vptxklxiscmxrcmkckk/database/query', {
      method: 'POST',
      headers: {
    ...
    if (res.ok) {
      ...
      console.log('   ⚠  Management API requer token pessoal. Schema precisa ser executado manualmente.')
      console.log('   → Abra o arquivo supabase/schema.sql e cole no SQL Editor do Supabase.')
      console.log('   → Após executar o schema, rode este script novamente para criar os admins.')
      console.log('\n   Resposta da API:', txt.slice(0, 200))
    }
  } catch (e) {
    console.log('   ⚠  Não foi possível criar schema automaticamente:', e.message)
  }

  // 2. Cria os admins (funciona independente do schema via trigger)
  console.log('\n2. Criando contas de administrador...')
  try {
    await criarAdmin('NeuroFlux', 'neuroflux', '607652', 'admin_mestre')
    await criarAdmin('Rogério',   'rogerio',   'kaizen',  'admin_sensei')
    console.log('\n   ✓ Admins prontos!')
    console.log('\n   Admin Mestre  → login: NeuroFlux  |  senha: 607652')
    console.log('   Admin Sensei  → login: Rogerio    |  senha: Kaizen')
  } catch (e) {
    console.log('   ✗ Erro:', e.message)
    console.log('   Execute o schema.sql primeiro e tente novamente.')
  }
